...
Bump rationale: First adoption of the project constitution; establishes seven core
principles plus constraints and workflow sections. MAJOR baseline.
...
Templates status:
  ✅ .specify/templates/plan-template.md  — "Constitution Check" derives gates from this
       file dynamically ([Gates determined based on constitution file]); no edit required.
  ✅ .specify/templates/spec-template.md  — no constitution-specific slots; aligned.
  ✅ .specify/templates/tasks-template.md — no constitution-specific slots; aligned.
  ✅ README.md — principles mirror README §3 (limits), §8 (DPDP), §10 (non-goals); aligned.
...
limits, token lifecycle (e.g. IG ~60-day refresh), and a contingency for deprecation.
...
- **Stack:** Supabase (Postgres + pgvector, Auth + RLS, Storage, Realtime, pgmq + pg_cron,
  ...
  **OpenRouter → Gemini 2.5 Flash** (Flash-Lite for bulk, Flash for nuanced); embeddings from
  a Gemini embedding model written into pgvector. A Render background worker is the only
  sanctioned escape hatch, added solely if Edge Function runtime limits block ingestion.
- **Secrets:** OAuth client secrets and per-cadre tokens MUST be encrypted at rest
  (Supabase Vault); never logged in plaintext.
